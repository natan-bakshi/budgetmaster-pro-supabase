...
const FunctionDisplay = ({ toolCall }) => {
    ...
    const statusConfig = {
        ...
        running: { icon: Loader2, color: 'text-slate-500', text: 'מעבד...', spin: true },
        in_progress: { icon: Loader2, color: 'text-slate-500', text: 'מעבד...', spin: true },
        completed: isError ? 
            { icon: AlertCircle, color: 'text-red-500', text: 'נכשל' } : 
            { icon: CheckCircle2, color: 'text-green-600', text: 'הצלחה' },
        success: { icon: CheckCircle2, color: 'text-green-600', text: 'הצלחה' },
...
export default function MessageBubble({ message }) {
    ...
    return (
        <div className={cn("flex gap-3", isUser ? "justify-end" : "justify-start")}>
            {!isUser && (
                <div className="h-7 w-7 rounded-lg bg-gradient-to-r from-blue-500 to-blue-600 flex items-center justify-center mt-0.5">
                    <div className="text-white font-bold text-sm">מ</div>
                </div>
            )}
